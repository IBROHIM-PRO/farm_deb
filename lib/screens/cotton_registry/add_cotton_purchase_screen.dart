...
class _AddCottonPurchaseScreenState extends State<AddCottonPurchaseScreen> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Дар як харидан то 3 навъи пахта (Линт, Улук, Валакно) дохил карда мешавад.',
              style: TextStyle(
                fontSize: 14,
      ...
      children: [
        ...
        ListView.separated(
          ...
          itemBuilder: (context, index) {
            return _buildCottonItemCard(index);
          },
        ),
        
        const SizedBox(height: 12),
        
        // Add/Remove Cotton Item Buttons
        Row(
          children: [
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${item.totalPrice.toStringAsFixed(0)} сомонӣ',
                          style: const TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                            color: Colors.green,
  ...
  Widget _buildCottonTypeChip(
    ...
    return GestureDetector(
      onTap: isDisabled ? null : () {
        setState(() {
          _cottonItems[itemIndex].cottonType = type;
        });
      },
      child: Container(
  ...
  Widget _buildPurchaseSummary() {
    final totalWeight = _cottonItems.fold(0.0, (sum, item) {
      return sum + (double.tryParse(item.weightController.text) ?? 0);
    });
    
    final totalUnits = _cottonItems.fold(0, (sum, item) {
      return sum + (int.tryParse(item.unitsController.text) ?? 0);
    });
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  '${subtotal.toStringAsFixed(0)} сомонӣ',
                  style: const TextStyle(fontWeight: FontWeight.bold),
                ),
              ],
            ),
            ...
            if (transportCost > 0) ...[
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    '${transportCost.toStringAsFixed(0)} сомонӣ',
                    style: const TextStyle(fontWeight: FontWeight.bold),
                  ),
                ],
              ),
            ...
            Row(
              ...
              children: [
                ...
                Text(
                  '${grandTotal.toStringAsFixed(0)} сомонӣ',
                  style: const TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                    color: Colors.green,
  ...
  void _calculateTotalPrice(int index) {
    ...
    setState(() {
      item.totalPrice = weight * pricePerKg;
    });
  }
      ...
      for (final item in _cottonItems) {
        if (item.weightController.text.trim().isEmpty ||
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Ҳама маълумоти пахтаро пур кунед'),
      ...
      try {
        ...
        final items = _cottonItems.map((itemData) {
          ...
          return CottonPurchaseItem(
            purchaseId: 0, // Will be set by provider
            cottonType: itemData.cottonType,
            weight: weight,
            units: int.parse(itemData.unitsController.text),
            pricePerKg: pricePerKg,
            totalPrice: weight * pricePerKg,
            notes: itemData.notesController.text.trim().isNotEmpty 
                ? itemData.notesController.text.trim() 
                : null,
          );
        }).toList();

        await context.read<CottonRegistryProvider>().addCottonPurchase(
          registry: registry,
          items: items,
        );
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Харидании пахта бомуваффақият сабт шуд'),
              backgroundColor: Colors.green,
            ),
          );
